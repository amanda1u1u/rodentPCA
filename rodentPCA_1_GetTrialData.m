...
t_peri = input('Enter perievent time (s): ');
t_margin = input('Enter extra margin time (s): ');
...
dataname = dir(fullfile(myPath, '*.nex'));
...
for d = 1:length(filenames)
    ...
    type_trial(is_failure) = {'FAILURE'};
    type_trial(~is_failure) = {'SUCCESS'};
